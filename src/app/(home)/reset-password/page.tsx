...
export default function CreatePasswordPage() {
  ...
  const validatePassword = (value: string) => {
    ...
    if (value.length < 8 || value.length > 10) {
      setErrors((prev) => ({ ...prev, password: "Password must be 8-10 characters long" }))
      return false
    }
    ...
    <main className="min-h-screen bg-[#760C2A] flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6 md:p-8">
        ...
        <div className="flex items-center mb-4">
          ...
          <h1 className="text-lg font-medium text-center flex-1 pr-5">Create New Password</h1>
        </div>

        {/* Subtitle */}
        <p className="text-sm text-gray-600 text-center mb-6">Your password must be 8-10 character long.</p>
