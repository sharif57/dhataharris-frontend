...
import { ArrowLeft } from "lucide-react";
import ImageUpload from "./ImageUploadProps";
import { useStoryCreateMutation } from "@/redux/feature/storySlice";
...
export default function CreateBlogModal({
  ...
  const [image, setImage] = useState<File | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [storyCreate] = useStoryCreateMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const formData = new FormData();
      formData.append("title", title);
      formData.append("description", content); // Matching the API key "description"
      if (image) {
        formData.append("image", image); // Matching the API key "image"
      }

      const res = await storyCreate(formData).unwrap();
      console.log(res, "res");
